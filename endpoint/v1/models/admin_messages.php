...
class admin_Messages {
    ...
    public function createNewConversation($receiver_id, $userid = null) {
        ...
        $sid = $db->where('sender_id', $uid)->where('receiver_id', (int) $receiver_id)->getOne('admin_conversations', array(
            'id',
            'created_at',
            'status'
        ));
        if ($sid['id'] > 0) {
            ...
            if($config->message_request_system == 'on') {
                if ((int)$sid['status'] == 2) {
                    if (intval($sid['created_at']) < strtotime("-1 day")) {
                        $data['status'] = 0;
                        $db->where('sender_id', $uid)->where('receiver_id', (int) $receiver_id)->update('admin_conversations',array('status'=>0));
                        $db->where('sender_id', (int) $receiver_id)->where('receiver_id', $uid)->update('admin_conversations',array('status'=>1));
                        $isnew = true;
                    }
                }
            }
            $db->where('id', $sid['id'])->update('admin_conversations', $data);
        } else {
            $dat = array(
                'sender_id' => $uid,
            ...
            $db->insert('admin_conversations', $dat);
            $isnew = true;
        }
        $rid = $db->where('sender_id', (int) $receiver_id)->where('receiver_id', $uid)->getOne('admin_conversations', array(
            'id'
        ));
        if ($rid['id'] > 0) {
            $db->where('id', $rid['id'])->update('admin_conversations', array(
                'created_at' => time()
            ));
        } else {
            $dat2 = array(
            ...
            $db->insert('admin_conversations',$dat2 );
            $isnew = true;
        }
        if($isnew  === true){
            //  no notification for admin admin_conversations
            // if($config->message_request_system == 'on'){
    ...
    public function getUnreadMessages() {
        ...
        $messages           = $db->objectBuilder()->where('`to`', auth()->id)->where('`from`', $blocked_user_array, 'NOT IN')->where('seen', 0)->getValue('admin_messages', 'count(*)');
        return $messages;
    }
    public function setMessageSeen($id, $userid) {
        ...
        $message = $db->where('id', $id)->where('`from`', $userid)->update('admin_messages', array(
            'seen' => time()
        ));
        if ($message) {
            return true;
    ...
    public function getConversationList($userid = null, $limit = null, $offset = 0) {
        ...
        $receiver           = $db->objectBuilder()->where('id  > ' . (int) $offset)->where('sender_id', $uid)
            ...
            ->get('admin_conversations', $limit, array('id','receiver_id'));
        foreach ($receiver as $key => $value) {
            $msg = $db->objectBuilder()->Where('`from_delete`', '0')->where('`from`', $uid)->Where('`to`', $value->receiver_id)->where('`to`', $blocked_user_array, 'NOT IN')->orderBy('id', 'DESC')->get('admin_messages', 1, array(
                '`text`',
                'from_delete',
                'to_delete',
                'seen',
            ...
            if (!$msg) {
                $msg = $db->objectBuilder()->Where('to_delete', '0')->where('`to`', $uid)->Where('`from`', $value->receiver_id)->where('`from`', $blocked_user_array, 'NOT IN')->orderBy('id', 'DESC')->get('admin_messages', 1, array(
                    '`text`',
                    'from_delete',
                    'to_delete',
                    'seen',
            ...
            if (isset($msg[0])) {
                ...
                if (count( (array)$user) > 0) {
                    ...
                    $new_messages = $db->where('`from`',$value->receiver_id)->where('`to`',$lm[0]->from)->where('seen',0)->getOne('admin_messages','count(*) as ct');
                    $dt = array(
    ...
    public function searchChat($search_text, $is_online = 0) {
        ...
        $conversationList = array();
        $sql              = 'SELECT 
                  admin_conversations.sender_id,
                  admin_conversations.created_at,
                  (SELECT MAX(messages.`created_at`) AS created_at FROM messages WHERE `messages`.`from` = admin_conversations.sender_id AND `messages`.`to` = 3) AS ct,
                  users.username,
                  users.first_name,
                  users.last_name,
                  users.avater,
                  users.lastseen,
                  (SELECT COUNT(*) FROM mediafiles WHERE mediafiles.user_id = admin_conversations.sender_id) AS media_count,
                  users.admin,
                  users.phone_verified,
                  users.active
                FROM
                admin_conversations
                  INNER JOIN users ON (admin_conversations.sender_id = users.id)
                WHERE
                  users.`verified` = \'1\' AND
                  admin_conversations.receiver_id = ' . auth()->id . ' AND 
                  admin_conversations.from_delete = 0 AND 
                  ((users.username LIKE \'%' . $search_text . '%\') OR CONCAT( users.first_name,  \' \', users.last_name ) LIKE  \'%' . $search_text . '%\') AND
                  admin_conversations.sender_id NOT IN (SELECT `block_userid` FROM `blocks` WHERE `user_id` = ' . auth()->id . ')';
    ...
    {
        global $db,$config;
        $updated = $db->where('`to`', auth()->id)->where('seen', 0)->update('admin_messages', array(
            'seen' => time()
        ));
        return json(array(
            'message' => 'SUCCESS',
    ...
    public function messages_requests()
    {
        ...
        if (!empty($_POST['user_id']) && !empty($_POST['type']) && in_array($_POST['type'], ['accept','decline']) && !empty(auth())) {
            ...
            if ($_POST['type'] == 'accept') {
                $status = $db->where('sender_id', auth()->id)->where('receiver_id', $user_id)->update('admin_conversations',array('status' => '1'));
                $status = $db->where('sender_id', $user_id)->where('receiver_id', auth()->id)->update('admin_conversations',array('status' => '1'));
                ...
                $db->where('sender_id', auth()->id)->where('receiver_id', $user_id)->delete('admin_conversations');
                $db->where('sender_id', $user_id)->where('receiver_id', auth()->id)->delete('admin_conversations');
                $db->where("(`to` = '".auth()->id."' AND `from` = '".$user_id."') OR (`from` = '".auth()->id."' AND `to` = '".$user_id."')")->delete('admin_messages');
                // $db->where('from', auth()->id)->where('to', $user_id)->delete('admin_messages');
                // $Notif = LoadEndPointResource('Notifications');
                // if($Notif) {
    ...
    public function send_text_message() {
        ...
        if (empty($_POST['message']) || empty($_POST['to_userid']) || $token == null) {
            ...
            $data  = array();
            $saved = $db->insert('admin_messages', array(
                'from' => (int) $user_id,
                'to' => (int) $to_userid,
                'text' => $message,
                'seen' => 0,
        ...
        if (empty($_FILES['media_file']) || empty($_POST['to_userid']) || $token == null) {
            ...
            foreach ($_FILES as $file) {
                ...
                if (move_uploaded_file($file['tmp_name'], $filename)) {
                    ...
                    $saved             = $db->insert('admin_messages', $msg);
                    if ($saved) {
                        $data['id']          = $saved;
                        $data['from']        = (int) $user_id;
                        $data['from_delete'] = 0;
    ...
    public function delete_messages() {
        ...
        if (empty($_POST['to_userid']) || empty($_POST['access_token'])) {
            ...
            $to_userid = (int) Secure($_POST['to_userid']);
            $db->where('`to`', $to_userid)->where('`from`', $user_id)->update('admin_messages', array(
                'from_delete' => '1'
            ));
            $db->where('`to`', $user_id)->where('`from`', $to_userid)->update('admin_messages', array(
                'to_delete' => '1'
            ));
            $db->where('sender_id', $to_userid)->where('receiver_id', $user_id)->update('admin_conversations', array(
                'from_delete' => '1'
            ));
            $db->where('sender_id', $user_id)->where('receiver_id', $to_userid)->update('admin_conversations', array(
                'to_delete' => '1'
            ));
            $db->where('from_delete', '1')->where('to_delete', '1')->delete('admin_messages');
            $db->where('from_delete', '1')->where('to_delete', '1')->delete('admin_conversations');
            return array(
                'status' => 200,
